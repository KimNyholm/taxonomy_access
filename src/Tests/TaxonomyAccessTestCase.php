  ...
  public function setUp() {
    ...
    foreach ($this->user_config as $user => $permissions) {
      ...
      foreach ($this->users[$user]->roles as $rid => $role) {
        if ($rid != \Drupal\Core\Session\AccountInterface::AUTHENTICATED_RID) {
    ...
    $rows = [];
    foreach ([\Drupal\Core\Session\AccountInterface::ANONYMOUS_ROLE, \Drupal\Core\Session\AccountInterface::AUTHENTICATED_RID] as $rid) {
      $ignore = [
